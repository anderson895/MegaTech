...
<div class="max-w-5xl mx-auto mt-12 p-8 bg-white rounded-2xl shadow-lg border border-gray-200 font-sans text-gray-800">
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
    <div class="text-center">
      ...
      <a href="../qrcodes/qr_<?= $order['order_id'] ?>.png" download class="mt-3 inline-flex items-center px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-lg shadow transition">
    ...
    <div class="text-center">
      ...
      <a href="../proofPayment/<?= $order['order_down_payment_receipt'] ?>" download class="mt-3 inline-flex items-center px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-lg shadow transition">
        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round"
                d="M4 16v2a2 2 0 002 2h12a2 2 0 002-2v-2M7 10l5 5m0 0l5-5m-5 5V4"></path>
        </svg>
  ...
  <?php if ($order['order_status'] !== 'pickedup') : ?>
    <form action="backend/update_status.php" method="POST" class="mt-12 text-center">
      <input type="hidden" name="order_id" value="<?= $order['order_id'] ?>">
      <input type="hidden" name="new_status" value="picked up">
      <button type="submit"
        class="inline-flex items-center px-6 py-3 bg-green-600 hover:bg-green-700 text-white text-base font-semibold rounded-xl shadow-lg transition orderActionToggler"
  ...
  <?php else : ?>
    <div class="mt-12 text-center text-green-700 font-semibold text-lg">
